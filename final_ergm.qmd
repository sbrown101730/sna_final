...
load("sna_final_centralities.RData")
...
summary(askreddit_giant ~ edges)                     # Number of edges (ties)
summary(askreddit_giant ~ mutual)                    # Number of pairs of reciprocated ties
summary(askreddit_giant ~ odegree(0:5))              # Outdegree distribution. (# of nodes with outdegree of 0, # nodes outdegree of 1, etc.)
                                           # Remember, respondents could nominate at most five employees in our survey
summary(askreddit_giant ~ idegree(0:65))             # Indegree distribution.
summary(askreddit_giant ~ gwodegree(log(2),fixed=T)) # One parameter summarizing outdegree distribution - tendency against outdegree hubs
summary(askreddit_giant ~ gwidegree(log(2),fixed=T)) # One parameters summarizing indegree distribution - tendency against indegree hubs
summary(askreddit_giant ~ desp(1:5))  

askreddit_giant
# Pairs of nodes with one shared partner, two shared partners, etc.
# summary(askreddit_giant ~ dgwesp(log(2),fixed = T))  # One parameter summarizing 

# # Look at Exogenous statistics: terms based on advice ties AND other ties / node attributes
# summary(askreddit_net ~ edgeicov("sentiment"))             # Ties directed towards employees at the main office (as opposed to secondary office)
# summary(advice ~ nodeocov("office"))             # Ties originating from employees at the main office (as opposed to secondary office)

# remove missing
summary(askreddit_giant ~ nodematch("popularity", levels = -3, diff = TRUE)) 

# Number of ties between people of the same sex
 # summary(askreddit_net ~ nodematch("size"))        # Number of ties between people working in the same department
...
model1 <- ergm(askreddit_giant ~ edges          # This is  a tendency towards a greater number of advice ties existing. Based on a statistic counting the number of ties.
               # Structural patterns
               + mutual 
...
advice.tri <- summary(askreddit_net ~ triangle)                                    # Stores the number of observed triangles
advice.tri
