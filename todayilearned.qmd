...
# Convert todayilearned_3 into an igraph object
todayilearned3_igraph <- igraph::graph_from_data_frame(todayilearned_3, directed = TRUE)

# As a first step, create a 'sna' graph object from an 'igraph' object
sna_todayilearned3 <-
  igraph::as_adjacency_matrix(todayilearned3_igraph, sparse = FALSE) %>% network::as.network.matrix()
...
```{r}
# Find the components
todayilearned_comp <- igraph::components(todayilearned3_igraph)

# Extract the largest connected component
giantGraph_todayilearned <- induced_subgraph(todayilearned3_igraph, which(todayilearned_comp$membership == which.max(todayilearned_comp$csize)))

# Verify the size of the largest component
vcount(giantGraph_todayilearned) # number of nodes
ecount(giantGraph_todayilearned) # number of edges


# We will compute centralities based on 'network' package
# First, create a dataframe to store the centrality information
centralities_giantGraph_todayilearned <- data.frame('node_name' = V(giantGraph_todayilearned)$name)
```
...
V(giantGraph_todayilearned)$in_degree <- igraph::degree(giantGraph_todayilearned, mode = "in")

# Calculate out-degree centrality (number of outgoing edges)
V(giantGraph_todayilearned)$out_degree <- igraph::degree(giantGraph_todayilearned, mode = "out")

# Calculate total degree centrality (sum of in-degree and out-degree)
V(giantGraph_todayilearned)$total_degree <- igraph::degree(giantGraph_todayilearned, mode = "all")

# Display the top 10 nodes by in-degree
top_in_degree <- data.frame(node_name = V(giantGraph_todayilearned)$name, in_degree = V(giantGraph_todayilearned)$in_degree) |> 
  dplyr::slice_max(order_by = in_degree, n = 10) |> 
  kableExtra::kable()
top_in_degree

# Display the top 10 nodes by out-degree
top_out_degree <- data.frame(node_name = V(giantGraph_todayilearned)$name, out_degree = V(giantGraph_todayilearned)$out_degree) |> 
  dplyr::slice_max(order_by = out_degree, n = 10) |> 
  kableExtra::kable()
top_out_degree

# Display the top 10 nodes by total degree
top_total_degree <- data.frame(node_name = V(giantGraph_todayilearned)$name, total_degree = V(giantGraph_todayilearned)$total_degree) |> 
...
V(giantGraph_todayilearned)$betweenness <- igraph::betweenness(giantGraph_todayilearned, directed = TRUE)

# Top 10 nodes by betweenness centrality
top_betweenness <- data.frame(node_name = V(giantGraph_todayilearned)$name, betweenness = V(giantGraph_todayilearned)$betweenness) |> 
...
V(giantGraph_todayilearned)$closeness <- igraph::closeness(giantGraph_todayilearned, mode = "out")

# Top 10 nodes by closeness centrality
top_closeness <- data.frame(node_name = V(giantGraph_todayilearned)$name, closeness = V(giantGraph_todayilearned)$closeness) |> 
...
V(giantGraph_todayilearned)$eigenvector <- igraph::eigen_centrality(giantGraph_todayilearned, directed = TRUE)$vector

# Top 10 nodes by eigenvector centrality
top_eigenvector <- data.frame(node_name = V(giantGraph_todayilearned)$name, eigenvector = V(giantGraph_todayilearned)$eigenvector) |> 
...
centralities_giantGraph_todayilearned$netconstraint <- igraph::constraint(giantGraph_todayilearned)
...
centralities_giantGraph_todayilearned |> 
...
## calculate k-cores
kcore_todayilearned <-
  giantGraph_todayilearned %>% coreness(.)
kcore_todayilearned ## show the results of k-core decomposition
...
giantGraph_todayilearned %>%
  plot(
    ...
    edge.arrow.size = .3,
    vertex.size = 20,
    vertex.label = V(giantGraph_todayilearned)$name,
...
cluster_todayilearned <- giantGraph_todayilearned %>% cluster_edge_betweenness()
...
#| label: modularity
# modularity measure
modularity(cluster_todayilearned)
...
cluster_todayilearned %>% plot(
  .,
  giantGraph_todayilearned,
  #layout = layout_nicely(giantGraph_gpt),
  layout = layout_with_fr(giantGraph_todayilearned),
  edge.arrow.size = .3,
  vertex.size = 10,
  vertex.label = V(todayilearned3_igraph)$name,
