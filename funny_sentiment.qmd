...
# find all subreddits with "funny" as source or target
funny_1 <- hyperlink_large %>%
  janitor::clean_names() %>%
  filter(source_subreddit == "funny" | target_subreddit == "funny")
  
# 2 layer: find all links from subreddits related to "funny"
...
popularity <- popularity_clean %>%
  filter(subreddit_name %in% funny$source_subreddit | 
           subreddit_name %in% funny$target_subreddit) %>%
...
funny_net <- as.network.matrix(funny, matrix.type = "edgelist") 
...
network::get.edge.attribute(funny_net,"sentiment")
...
funny_igraph <- graph.adjacency(as.matrix.network(funny_net), weighted = TRUE) # make an igraph network object from statnet network object

funny_igraph <- set_edge_attr(funny_igraph, "sentiment", value = funny$sentiment)

count_components(funny_igraph)
net_layout <- layout_with_fr(funny_igraph) # Calculates and stores a spring-embedded layout
                                           # We will re-use this same layout for each plot, so nodes are always in the same place
plot(funny_igraph, layout=net_layout, edge.color= "blue", vertex.label = NA)
```


```{r}
# Plot the Advice network with node coloring based on sex
E(funny_igraph)$color = ifelse(E(funny_igraph)$sentiment > 0.5, "green", "red")

plot(funny_igraph, layout=net_layout, vertex.label = V(funny_igraph)$name, vertex.label.cex = 0.5)
```
