...
# find all subreddits with "science" as source or target
science_1 <- hyperlink_large %>%
  janitor::clean_names() %>%
  filter(source_subreddit == "science" | target_subreddit == "science")
  
# 2 layer: find all links from subreddits related to "science"
...
science_net <- as.network.matrix(science_3, matrix.type = "edgelist") 
...
network::get.vertex.attribute(science_net,"vertex.names")

network::get.vertex.attribute(science_net,"size")
...
science_igraph <- graph.adjacency(as.matrix.network(science_net)) 

# set vertex attribute for size
science_igraph <- set_vertex_attr(science_igraph, "popularity", value = network::get.vertex.attribute(science_net,"size"))

# count components
count_components(science_igraph)
...
reddit_comp <- igraph::components(science_igraph)
giantGraph_science <- science_igraph %>%
  induced_subgraph(., which(reddit_comp$membership == which.max(reddit_comp$csize)))

plot(giantGraph_science)

library(intergraph)

science_giant <- asNetwork(giantGraph_science)
...
summary(science_giant ~ edges)                     # Number of edges (ties)
summary(science_giant ~ mutual)                    # Number of pairs of reciprocated ties

summary(science_giant ~ idegree(0:65))             # Indegree distribution.
summary(science_giant ~ gwodegree(log(2),fixed=T)) # One parameter summarizing outdegree distribution - tendency against outdegree hubs
summary(science_giant ~ gwidegree(log(2),fixed=T)) # One parameters summarizing indegree distribution - tendency against indegree hubs
summary(science_giant ~ desp(1:5))  

science_giant

# remove missing
summary(science_giant ~ nodematch("popularity", levels = -3, diff = TRUE)) 
```
...
## 3. Subreddits have a preferential indegree attachement --> want to determine whether science is a hub or an authority node.
...
model1 <- ergm(science_giant ~ edges
               + mutual 
...
advice.tri <- summary(science_net ~ triangle)                                    # Stores the number of observed triangles
advice.tri
