...
summary(worldnews_giant ~ edges)                     # Number of edges (ties)
summary(worldnews_giant ~ mutual)                    # Number of pairs of reciprocated ties

summary(worldnews_giant ~ idegree(0:65))             # Indegree distribution.
summary(worldnews_giant ~ gwodegree(log(2),fixed=T)) # One parameter summarizing outdegree distribution - tendency against outdegree hubs
summary(worldnews_giant ~ gwidegree(log(2),fixed=T)) # One parameters summarizing indegree distribution - tendency against indegree hubs
summary(worldnews_giant ~ desp(1:5))  

worldnews_giant

# remove missing
summary(worldnews_giant ~ nodematch("popularity", levels = -3, diff = TRUE)) 
```
...
## 3. Subreddits have a preferential indegree attachement --> want to determine whether worldnews is a hub or an authority node.
