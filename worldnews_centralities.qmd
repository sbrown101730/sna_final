...
# Convert worldnews_3 into an igraph object
worldnews3_igraph <- igraph::graph_from_data_frame(worldnews_3, directed = TRUE)

# As a first step, create a 'sna' graph object from an 'igraph' object
sna_worldnews3 <-
  igraph::as_adjacency_matrix(worldnews3_igraph, sparse = FALSE) %>% network::as.network.matrix()
...
```{r}
# Find the components
worldnews_comp <- igraph::components(worldnews3_igraph)

# Extract the largest connected component
giantGraph_worldnews <- induced_subgraph(worldnews3_igraph, which(worldnews_comp$membership == which.max(worldnews_comp$csize)))

# Verify the size of the largest component
vcount(giantGraph_worldnews) # number of nodes
ecount(giantGraph_worldnews) # number of edges


# We will compute centralities based on 'network' package
# First, create a dataframe to store the centrality information
centralities_giantGraph_worldnews <- data.frame('node_name' = V(giantGraph_worldnews)$name)
```
...
V(giantGraph_worldnews)$in_degree <- igraph::degree(giantGraph_worldnews, mode = "in")

# Calculate out-degree centrality (number of outgoing edges)
V(giantGraph_worldnews)$out_degree <- igraph::degree(giantGraph_worldnews, mode = "out")

# Calculate total degree centrality (sum of in-degree and out-degree)
V(giantGraph_worldnews)$total_degree <- igraph::degree(giantGraph_worldnews, mode = "all")

# Display the top 10 nodes by in-degree
top_in_degree <- data.frame(node_name = V(giantGraph_worldnews)$name, in_degree = V(giantGraph_worldnews)$in_degree) |> 
  dplyr::slice_max(order_by = in_degree, n = 10) |> 
  kableExtra::kable()
top_in_degree

# Display the top 10 nodes by out-degree
top_out_degree <- data.frame(node_name = V(giantGraph_worldnews)$name, out_degree = V(giantGraph_worldnews)$out_degree) |> 
  dplyr::slice_max(order_by = out_degree, n = 10) |> 
  kableExtra::kable()
top_out_degree

# Display the top 10 nodes by total degree
top_total_degree <- data.frame(node_name = V(giantGraph_worldnews)$name, total_degree = V(giantGraph_worldnews)$total_degree) |> 
...
V(giantGraph_worldnews)$betweenness <- igraph::betweenness(giantGraph_worldnews, directed = TRUE)

# Top 10 nodes by betweenness centrality
top_betweenness <- data.frame(node_name = V(giantGraph_worldnews)$name, betweenness = V(giantGraph_worldnews)$betweenness) |> 
...
V(giantGraph_worldnews)$closeness <- igraph::closeness(giantGraph_worldnews, mode = "out")

# Top 10 nodes by closeness centrality
top_closeness <- data.frame(node_name = V(giantGraph_worldnews)$name, closeness = V(giantGraph_worldnews)$closeness) |> 
...
V(giantGraph_worldnews)$eigenvector <- igraph::eigen_centrality(giantGraph_worldnews, directed = TRUE)$vector

# Top 10 nodes by eigenvector centrality
top_eigenvector <- data.frame(node_name = V(giantGraph_worldnews)$name, eigenvector = V(giantGraph_worldnews)$eigenvector) |> 
...
centralities_giantGraph_worldnews$netconstraint <- igraph::constraint(giantGraph_worldnews)
...
centralities_giantGraph_worldnews |> 
...
## calculate k-cores
kcore_worldnews <-
  giantGraph_worldnews %>% coreness(.)
kcore_worldnews ## show the results of k-core decomposition
...
giantGraph_worldnews %>%
  plot(
    ...
    edge.arrow.size = .3,
    vertex.size = 20,
    vertex.label = V(giantGraph_worldnews)$name,
...
cluster_worldnews <- giantGraph_worldnews %>% cluster_edge_betweenness()
...
cluster_worldnews %>% plot(
  .,
  giantGraph_worldnews,
  #layout = layout_nicely(giantGraph_gpt),
  layout = layout_with_fr(giantGraph_worldnews),
  edge.arrow.size = .3,
  vertex.size = 10,
  vertex.label = V(worldnews3_igraph)$name,
